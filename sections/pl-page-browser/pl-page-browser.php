...
/*
  ...
  Plugin Name:    PL Framework Page Browser
  Description:    Easily browse pages, posts, and everything else. 
...
class PL_Page_Browser {
  ...
  function __construct(){
    ...
    $this->id = 'pl-page-browser';
    $this->url = get_stylesheet_directory_uri() . '/sections/'.$this->id;

    add_action('wp', array($this, 'setup_ui' )) ; 


    add_action('pl_workarea_scripts', array($this, 'scripts'));
    add_action('pl_workarea_footer', array($this, 'template'));

    add_filter('pl_workarea_json', array($this, 'data'));

    add_filter( 'pl_server_get_page_children',  array( $this, 'get_page_children' ),  10, 2 );
    add_filter( 'pl_server_search_pages',       array( $this, 'search_pages' ),       10, 2 );


   
  }

  /** After WP Query is loaded */
  function setup_ui(){
    ...
    pl_add_ab_menu( array(
      'id'      => 'pl-ab-browser',
      'title'   => '<i class="icon icon-list-alt"></i> Page Browser',
      'rel'     => 'plPageBrowser',
      'href'    => add_query_arg('pl_tool', 'plPageBrowser', PL()->urls->editor )
    ));
  }

  function data( $json ){

    $json['layouts'] = json_encode(array( 'pages' => $this->get_pages_array() ));
  ...
  function scripts(){
    ...
    wp_enqueue_style( $this->id, $this->url . '/style.css' );
...
<?php 
      printf('<script type="text/plain" id="layout-page-template">%s</script>', ob_get_clean() );
  ...
  <div id="layout-selector-select" class="pl-page-selector fix">
    ...
    <div id="layout-selector-select-content" class="selector-content">
      <strong id="currently-editing">Currently Editing:</strong><div id="current-layout"></div>  
    </div>
  ...
  <?php
      ...
      printf('<script type="text/plain" id="pl-page-browser-template">%s</script>', ob_get_clean() );
    ...
    function format_pages_array( $page_slug, $info ) {
      ...
      $page_setup = array(
        'id'          =>  $page_slug,
        'name'        =>   $this->array_get( 'name',    $info ) ? $this->array_get( 'name', $info ) : $plpg->get_page_slug_info( $page_slug, 'name' ),
        'url'         =>   $this->array_get( 'url',     $info ) ? $this->array_get( 'url', $info ) : $plpg->get_page_slug_info( $page_slug, 'url' ),
