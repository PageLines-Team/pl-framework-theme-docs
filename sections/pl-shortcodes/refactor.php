...
function pagelines_check_pro_nag( $note ) {
  ...
  <div class="alert editor-alert">
    ...
    <a href="http://www.pagelines.com/DMS" class="btn btn-xs" target="_blank"><i class="icon icon-thumbs-up"></i> <?php _e( 'Learn More About Pro', 'pagelines' ); ?>
    </a>
    </div>
    <?php
    $note .= ob_get_clean();
...
function pagelines_add_admin_menus() {
  ...
  global $pl_admin_actions;
  
  $_pagelines_account_hook = pagelines_insert_menu( PL_MAIN_DASH, __( 'Settings', 'pagelines' ), 'edit_theme_options', PL_MAIN_DASH, array($pl_admin_actions,'pagelines_build_account_interface') );
...
function pagelines_add_admin_menu() {
    ...
    $menu['3.3'] = array( '', 'edit_theme_options', 'separator-pagelines', '', 'wp-menu-separator' );

    add_menu_page( __( 'PageLines', 'pagelines' ), __( 'PageLines', 'pagelines' ), 'edit_theme_options', PL_MAIN_DASH, array($pl_admin_actions, 'pagelines_build_account_interface'), PL_IMAGES . '/admin-icon.png', '3.4' );
}
